...
export default function DecipherScrollScreen() {
  ...
  const awardHintFromRealAd = async () => {
    try {
      ...
      console.warn("[DecipherScroll] Failed to award hint after ad:", e);
    }
  };
  ...
  // Fetch riddle from Groq
  useEffect(() => {
    let isMounted = true;
    const fetchChallenge = async () => {
      try {
        const status = await AsyncStorage.getItem("quest_decipher_scroll_status");
        ...
        console.log("[DecipherScroll] Generating Super Hard Riddle...");
        const result = await generatePuzzle("paheli", undefined, "Super Hard");
        if (isMounted) {
          setPuzzle(result);
        ...
        console.warn("[DecipherScroll] Failed to generate Groq puzzle, falling back...", e);
        // Fallback to local
        const fallback: Puzzle = {
          id: "ds_fallback_" + Date.now(),
          category: "Nature",
          clue: "I have no voice, yet I speak. I have no wings, yet I fly. I bring tears, yet I have no feelings. What am I?",
          answer: "CLOUD",
          decoys: ["M", "B", "R"],
          hint: "Seen in the sky during the monsoons.",
        };
        if (isMounted) {
          setPuzzle(fallback);
      ...
      try {
        ...
        console.warn("[DecipherScroll] Failed to load hints:", err);
      }
    };
    fetchChallenge();
  ...
  useEffect(() => {
    const handleAppState = (nextAppState: AppStateStatus) => {
      if (
        ...
        if (!loading && !isGameOver && !isWon) {
          console.log("[DecipherScroll] App minimized! Failing quest.");
  ...
  const handleGameFail = async (reason: "time_up" | "app_minimized" | "abandoned") => {
    ...
    try {
      ...
      await AsyncStorage.setItem("quest_decipher_scroll_status", "played");
      await saveQuestStatus("decipher_scroll", "played", 0);
    ...
    if (reason === "app_minimized" || reason === "abandoned") {
      ...
      showMessage({
        message: "Time's Up! ⏰",
        description: "You could not decipher the scroll. Attempt lost!",
        type: "danger",
        duration: 3500,
      });
    }
  ...
  const handleSubmit = async () => {
    ...
    if (userWord === correctWord && puzzle) {
      ...
      try {
        ...
        await AsyncStorage.setItem("quest_decipher_scroll_status", "completed");
        await saveQuestStatus("decipher_scroll", "completed", 100);
        // Award +100 XP
        await awardXP(100, "daily_quest_completed", puzzle.id);
      ...
      showMessage({
        ...
        description: "Scroll deciphered! You have received +100 XP!",
        type: "success",
        duration: 3000,
      });
    } else {
      ...
      showMessage({
        ...
        description: "Check the riddle answer by placing the correct tiles!",
        type: "danger",
        duration: 1500,
      });
    }
  ...
  const BeautifulLoader = () => {
    ...
    return (
      <LinearGradient colors={["#0A1D1A", "#143D36"]} style={styles.container}>
        <View style={styles.loaderCenter}>
          <Animated.View style={{ transform: [{ rotate: spin }] }}>
            <Ionicons name="document-text" size={64} color="#10B981" />
          </Animated.View>
          <Text style={styles.loaderText}>TRANSLATING ANCIENT SCROLL...</Text>
          <Text style={styles.loaderSubText}>Generating cryptic riddle challenge...</Text>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerTitleContainer}>
            <Text style={styles.headerTitle}>Decipher Scroll 📜</Text>
        ...
        <View style={styles.clueCard}>
          <LinearGradient colors={["#163530", "#214C45"]} style={styles.clueGradient}>
            <Text style={styles.clueQuote}>"{puzzle.clue}"</Text>
            <View style={styles.cardDivider} />
            <Text style={styles.clueMeta}>Letters: {puzzle.answer.length} • Difficulty: Super Hard Riddle</Text>
          </LinearGradient>
        </View>

        {/* Selected Answer Slots */}
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>DECODED SCROLL SOLUTION</Text>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>RIDDLE TILES</Text>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
            <Text style={styles.sectionMeta}>Available letters</Text>
            <TouchableOpacity
        ...
        {isWon || isGameOver ? (
          ...
          <View style={styles.footerActionContainer}>
            ...
            >
              ...
              <Text style={styles.submitBtnText}>SUBMIT SCROLL ANSWER</Text>
            </TouchableOpacity>
        ...
        {isGameOver && (
          <View style={styles.overlayOverlay}>
            <View style={styles.failCard}>
              ...
              <Text style={styles.overlayTitle}>Decipher Failed! ❌</Text>
              <Text style={styles.overlayDesc}>
                You could not decipher the riddle in time. The daily quest attempt is locked for today.
              </Text>
              <TouchableOpacity
                onPress={() => router.replace("/(authenticated)/(tabs)/quest")}
        ...
        {isWon && (
          <View style={styles.overlayOverlay}>
            <View style={styles.victoryCard}>
              <FontAwesome5 name="feather-alt" size={56} color="#10B981" />
              <Text style={styles.overlayTitle}>Riddle Deciphered! 🎉</Text>
              <Text style={styles.overlayPoints}>+100 XP Earned</Text>
...
const styles = StyleSheet.create({
  ...
  overlayOverlay: {
    ...
    backgroundColor: "rgba(10, 29, 26, 0.95)",
    justifyContent: "center",
    alignItems: "center",
    padding: 24,
  },
